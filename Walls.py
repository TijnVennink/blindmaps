...
def create_wall(topx, topy, height, width):
    wall = pygame.Rect(topx, topy, height, width)
    return wall

topx, topy, heightwall, widthwall = input()
wall = create_wall(topx, topy, heightwall)
...
while run:
    ...
    spring_force = -k_virtual * (
                np.array(center) - np.array(xh))  # Calculate the force applied on the device by the spring
    # fe += spring_force
